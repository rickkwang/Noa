...
describe('light theme border tokens', () => {
  ...
  it('starts the lifted tab-strip baseline after the visible desktop sidebar', async () => {
    ...
    expect(topBar).toContain("!isMobile && isSidebarVisible ? 'after:left-[var(--noa-sidebar-width,320px)]' : 'after:left-0'");
  ...
  it('animates the separator with direct toggles but keeps it fixed during preview promotion', async () => {
    ...
    expect(app).toContain(": isSidebarOpen ? 'var(--noa-sidebar-width, 320px)' : '-1px'");
  ...
  it('slides fixed-width side panels instead of cropping them with width animation', async () => {
    ...
    expect(app).toMatch(/marginLeft: !isMobile && !isPromotingSidebarPreview && \(isFocusMode \|\| !isSidebarOpen\)[\s\S]*?'calc\(-1 \* var\(--noa-sidebar-width, 320px\)\)'[\s\S]*?: '0px'/);
