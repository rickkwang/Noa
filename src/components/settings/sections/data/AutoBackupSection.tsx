  ...
  return (
    <SettingSection title="Automatic backup" description="Writes a full snapshot to a folder on your disk on first launch each day. Keeps the most recent backups and deletes the rest.">
          ...
          {hasBackupHandle ? (
            <>
              ...
              <button
                ...
                disabled={busy}
                className="px-3 py-1.5 text-xs font-bold bg-[#F9F9F7] border border-[#2D2D2B] hover:bg-[#EFEAE3] disabled:opacity-50"
              >
                Disconnect
              </button>
            </>
      ...
      {status === 'needs-reauth' && hasBackupHandle && (
        <div className="px-3 py-2 border border-[#EC9A3C] bg-[#EC9A3C]/10 flex items-center justify-between gap-3 text-xs">
          <span className="text-[#74491A] flex items-center gap-1.5">
            <AlertTriangle size={12} /> Folder permission was revoked.
          </span>
          <button
            onClick={() => { void onReconnect(); }}
            className="px-3 py-1 text-xs font-bold bg-[#EC9A3C] text-white border border-[#2D2D2B] hover:opacity-90"
          >
            Reconnect
          </button>
        </div>
      ...
      {status === 'error' && error && !bannerDismissed && (
        <div className="px-3 py-2 border border-[#D45555]/60 bg-[#D45555]/10 flex items-center gap-1.5 text-xs text-[#A93B3B]">
          <AlertTriangle size={12} /> {error}
        </div>
      )}

      {status === 'success' && !bannerDismissed && (
        <div className="px-3 py-2 border border-[#4CAF8A] bg-[#4CAF8A]/10 flex items-center gap-1.5 text-xs text-[#2C6E57]">
