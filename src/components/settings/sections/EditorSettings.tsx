          ...
          <div className="flex space-x-2" role="group" aria-label="Editor view mode">
            {(['edit', 'split', 'preview'] as const).map((mode) => (
              <button
                ...
                className={`px-3 py-1.5 font-bold border border-[#2D2D2B] rounded-md text-sm capitalize transition-colors ${
                  editorViewMode === mode
      ...
      <SettingSection title="Daily Notes" description="Configure automatic daily note creation.">
        ...
        <SettingItem label="Date Format" description="Format for the daily note title. Uses YYYY MM DD HH mm tokens.">
          <div className="space-y-1">
            <input
              ...
              className="bg-[#F9F9F7] border border-[#2D2D2B] rounded-md px-3 py-1.5 text-sm w-40 shadow-[inset_2px_2px_0px_0px_rgba(0,0,0,0.05)] outline-none focus:border-[#CC7D5E]"
            />
        ...
        <SettingItem label="Template" description="Content pre-filled in each new daily note. Supports {{date}}, {{title}}, {{time}}, {{week}}, {{weeknum}}.">
          <textarea
            ...
            className="bg-[#F9F9F7] border border-[#2D2D2B] rounded-md px-3 py-2 text-sm w-full font-redaction shadow-[inset_2px_2px_0px_0px_rgba(0,0,0,0.05)] outline-none focus:border-[#CC7D5E] resize-none"
          />
        </SettingItem>
      </SettingSection>
      ...
      <SettingSection title="Custom Templates" description="Create reusable note templates. Supports {{date}}, {{title}}, {{time}}, {{week}}, {{weeknum}}.">
        <div className="space-y-2">
          {userTemplates.map(t => (
            <div key={t.id} className="border border-[#2D2D2B] rounded-lg p-3 flex items-center justify-between gap-3">
              ...
              <div className="flex items-center gap-2 shrink-0">
                <button
                  onClick={() => openEdit(t)}
                  className="border border-[#2D2D2B] rounded-md px-2 py-1 text-xs active:opacity-70"
                >
                  Edit
                </button>
                {confirmDeleteId === t.id ? (
                  <button
                    ...
                    className="border border-[#C24444] text-[#C24444] rounded-md px-2 py-1 text-xs active:opacity-70"
                  >
                    Confirm?
                  </button>
                ) : (
          ...
          {editingId !== null && (
            <div
              ref={editFormRef}
              className="border border-[#2D2D2B] rounded-lg p-3 space-y-3 mt-2"
            ...
            >
              <div>
                ...
                <input
                  ...
                  className="bg-[#F9F9F7] border border-[#2D2D2B] rounded-md px-3 py-1.5 text-sm w-full outline-none focus:border-[#CC7D5E]"
                  autoFocus
                />
              </div>
              <div>
                ...
                <textarea
                  ...
                  className="bg-[#F9F9F7] border border-[#2D2D2B] rounded-md px-3 py-2 text-sm w-full font-redaction outline-none focus:border-[#CC7D5E] resize-none"
                />
              </div>
              <div className="flex gap-2">
                <button
                  ...
                  className="border border-[#2D2D2B] bg-[#2D2D2B] text-[#F9F9F7] rounded-md px-3 py-1 text-xs font-bold active:opacity-70 disabled:opacity-50 disabled:pointer-events-none"
                >
                  Save
                </button>
                <button
                  onClick={cancelEdit}
                  className="border border-[#2D2D2B] rounded-md px-3 py-1 text-xs font-bold active:opacity-70"
                >
                  Cancel
                </button>
              </div>
          ...
          {editingId === null && (
            <button
              onClick={openNew}
              className="border border-[#2D2D2B] rounded-md px-3 py-1.5 text-xs font-bold w-full text-left active:opacity-70 hover:bg-[#EFEAE3] mt-1"
            >
              + New Template
            </button>
          )}
