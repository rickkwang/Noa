  ...
  return (
    <div className="h-8 border-b border-[#2D2D2D] flex items-center px-4 shrink-0 bg-[#EAE8E0] z-10 font-redaction space-x-2 text-[#2D2D2D]/70 relative overflow-x-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
      <button
        ...
        className="p-1 hover:text-[#CC7D5E] hover:bg-[#DCD9CE]/50 active:opacity-70 transition-colors shrink-0"
        title="Bold"
      >
        <Bold size={14} />
      </button>
      ...
      <button
        onClick={() => onInsertFormatting('\n```\n', '\n```\n')}
        className="p-1 hover:text-[#CC7D5E] hover:bg-[#DCD9CE]/50 active:opacity-70 transition-colors text-xs font-bold shrink-0"
        title="Code Block"
      >
        {'</>'}
      </button>
      {hasToc && (
        <>
          ...
          <button
            onClick={onToggleToc}
            className={`p-1 hover:text-[#CC7D5E] hover:bg-[#DCD9CE]/50 active:opacity-70 transition-colors shrink-0 ${isTocOpen ? 'text-[#CC7D5E]' : ''}`}
            title="Outline"
          >
            <AlignLeft size={14} />
          </button>
