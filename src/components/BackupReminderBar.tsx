...
export default function BackupReminderBar({
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-1 bg-[#EAE8E0] border border-[#2D2D2D]/30 shadow-[2px_2px_0px_0px_rgba(45,45,45,0.15)] px-3 py-2 min-w-[280px]">
        <div className="flex items-center justify-between gap-4">
          <span className={`text-[11px] ${statusColor}`}>{message} · JSON backup every 7d</span>
          <button
            onClick={onDismiss}
            className="text-[#2D2D2D]/30 hover:text-[#2D2D2D]/70 active:opacity-70 transition-colors shrink-0"
        ...
        <div className="flex items-center justify-between gap-4">
          ...
          <button
            ...
            className="flex items-center gap-1 text-[10px] font-bold text-[#B89B5E] border border-[#B89B5E]/50 px-1.5 py-0.5 hover:bg-[#B89B5E]/10 active:opacity-70 transition-colors shrink-0"
          >
            <Download size={10} />
            Export
          </button>
